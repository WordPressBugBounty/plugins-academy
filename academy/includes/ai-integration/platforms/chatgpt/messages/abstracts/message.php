<?php
namespace Academy\AiIntegration\Platforms\Chatgpt\Messages\Abstracts;

use Academy\AiIntegration\Exceptions\{ PlaceholderMissingException, InvalidValueException };
		...
		if ( ! $this->is_filled && count( $missing_keys = array_diff( $this->placeholders, array_keys( $input ) ) ) === 0 ) {
			foreach ( $this->placeholders as $placeholder ) {
				if ( method_exists( $this, "validate_{$placeholder}" ) &&
					...
					throw new InvalidValueException( sprintf( __( 'Invalid value is provided for: %s.', 'academy-pro' ), $placeholder ) );
				}
			...
			throw new PlaceholderMissingException( sprintf( __( '%s fields are missing.', 'academy-pro' ), implode( ',', $missing_keys ) ) );
		}
		return [
			'role' => trim( $this->role ),
